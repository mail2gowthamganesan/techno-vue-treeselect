...
const Option = {
  ...
  methods: {
    ...
    renderCheckboxContainer(children) {
      ...
      if (node && node.raw && node.raw.checkBox === false) return null;
    ...
    renderLabel() {
      ...
      if(instance.showToolTip && !node.isRootNode)
        return (
          <popper class="d-block">
            ...
            <label class={labelClassName} slot="reference">
              {node.label}
              {shouldShowCount && <span class={countClassName}>({count})</span>}
            </label>
          </popper>
    ...
    handleMouseDownOnLabelContainer: onLeftClick(
      function handleMouseDownOnLabelContainer() {
        ...
        if (node.isBranch && instance.disableBranchNodes) {
          instance.toggleExpanded(node);
        } else if (node && node.raw && node.raw.checkBox === false) {
          instance.toggleExpanded(node);
        } else {
